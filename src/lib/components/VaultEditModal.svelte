<script>
	...
	let title = $state(vault.title);
	let privacy = $state(vault.privacy);
	let error = $state('');
	let apiError = $state('');
	...
	async function editVault() {
		...
		try {
			const response = await fetch(`${API_URL}/vaults/${vault.id}`, {
				method: 'PUT',
			...
			>
				...
				<div class="mt-10 flex items-center justify-end gap-2 px-4">
					<button class="px-2 py-1 text-sm hover:bg-zinc-700" onclick={modal.closeModal}
						>Cancel</button
					>
					<button class="bg-zinc-700 px-2 py-1 text-sm hover:bg-zinc-600" onclick={editVault}
						>Save</button
					>
				</div>
			</div>
