...
{#if vault}
	<div class="flex h-screen px-4 pb-10 pt-14">
		<div class="flex max-h-full w-full max-w-xs flex-col">
			<div class="flex border border-zinc-600 bg-zinc-800 px-4 py-4 pb-8">
				<div class="flex flex-col">
					<div class="mb-4 text-lg font-semibold">{vault.title}</div>
					<div class="mb-1 text-sm text-zinc-300">created {vault.time_since}</div>
					<div class="flex items-center gap-2">
						<div class="text-sm text-zinc-300">{vault.post_count} posts</div>
						<div class="text-sm text-zinc-300">|</div>
						<div class="text-sm text-zinc-300">{vault.privacy}</div>
					</div>
				</div>
